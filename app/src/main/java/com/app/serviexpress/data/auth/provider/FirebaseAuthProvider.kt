...
class FirebaseAuthProvider(private val auth: FirebaseAuth) : AuthenticationProvider {
    ...
    override suspend fun loginUser(email: String, password: String): AuthResult {
        return withContext(Dispatchers.IO) {
            try {
                ...
                when {
                    e.message?.contains("password is invalid") == true -> AuthResult.Failure(AuthError.InvalidCredentials)
                    e.message?.contains("no user record") == true -> AuthResult.Failure(AuthError.UserNotFound)
                    e.message?.contains("network") == true -> AuthResult.Failure(AuthError.NetworkError)
                    else -> AuthResult.Failure(AuthError.Unknown(e.message ?: "Error desconocido"))
                }
            }
        }
    }
    ...
    override suspend fun loginWithGoogle(idToken: String): AuthResult {
        return AuthResult.Failure(AuthError.Unknown("Método no implementado en FirebaseAuthProvider"))
    }
